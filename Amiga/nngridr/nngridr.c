...
void main(int argc, char **argv)
{
   ...
   WriteInit();
   Leader(indent);
   printf("..................... Execution Proceeding ......................\n");
   if (atime)
   {  time(&t1now);
      Leader(indent);
      printf("Execution began: %s\n",ctime(&t1now));
   }
   if (ReadData())
   {
      Leader(indent);         /* added data count printout 20 July 94 */
      printf("%d data were input\n",datcnt);
      if (ichoro)
      {  ChoroPleth();
         Leader(indent);
         if (densi) printf("Density calculations completed\n");
         else printf("Choropleth calculations completed\n");
      }
      if (igrad)
      {  Gradient();
         Leader(indent);
         printf("Gradient estimation completed\n");
      }
      MakeGrid();
      if (atime)
      {  time(&t2now);
      ...
      {  MakeToGif();
         GifMaker();
         Leader(indent);
         printf("%s file is complete\n",gif_file);
      }
      if (gnup) MakeGnufl();
      Leader(indent);
      printf("Grid completed, returned to OS\n");
   }
}
...
{
   datcnt = ioK = 0;
   dat_file = (char *) malloc(32 * sizeof(char));
   grd_file = (char *) malloc(32 * sizeof(char));
   gif_file = (char *) malloc(32 * sizeof(char));
   infform = (char *) malloc(32 * sizeof(char));
   indform = (char *) malloc(32 * sizeof(char));
   out1form = (char *) malloc(32 * sizeof(char));
   out1aform = (char *) malloc(48 * sizeof(char));
   out3form = (char *) malloc(48 * sizeof(char));
   out3aform = (char *) malloc(48 * sizeof(char));
   intstr = (char *) malloc(16 * sizeof(char));
}
...
void GetOptions(void)
{
   ...
   {  if (fscanf(filep, "%s", dat_file) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%s", grd_file) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &igrad) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &ichoro) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &densi) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &tgrid) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &x_nodes) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &y_nodes) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &non_neg) EQ EOF) 
         badfile = 1;
      ...
      if (fscanf(filep, "%d", &optim) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &arriba) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &bI) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &bJ) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &magx) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &magy) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &magz) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &xstart) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &ystart) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &xterm) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &yterm) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &horilap) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &vertlap) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%lf", &nuldat) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &extrap) EQ EOF) 
         badfile = 1;
      ...
      if (fscanf(filep, "%d", &y_extra) EQ EOF) 
         badfile = 1;
      if (fscanf(filep, "%d", &southhemi) EQ EOF) 
         badfile = 1;
      i0 = 0;
      if ((ch = fgetc(filep)) NE EOF)
      {  while ((ch = fgetc(filep)) NE EOF AND 
            ch NE '\n')
         {  indform[i0] = ch;
            i0++;
         }
      ...
      if (strlen(indform)<4) badfile = 1;
      if (strlen(infform)<6) badfile = 1;
      if (strlen(out1form)<2) badfile = 1;
      if (strlen(out1aform)<6) badfile = 1;
      if (strlen(out3form)<6) badfile = 1;
      if (strlen(out3aform)<10) badfile = 1;
      if (!badfile) ioK = 1;
      fclose(filep);
   }
   else badfile = 1;
   ...
   {  dat_file = "???????????";
      grd_file = "scratch.grd";
      igrad = ichoro = densi = tgrid = mxmn = 
         gnup = non_neg = sdip = idisplay =
         rads = optim = 0;
      arriba = magx = magy = magz = 1;
      bI = 1.5;
      bJ = 7.0;
      xstart = ystart = horilap = vertlap   = 1.0;
      xterm = yterm = 10.0;
      x_nodes = y_nodes   = 5;
      z_only = extrap = 1;
      indform = "%lf%lf%*s";
      infform = "%lf%lf%lf";
      out1form = "%12.4f";
      out1aform = "%12.4f %8.3f %8.3f";
      out3form = "%12.4f %12.4f %12.4f";
      out3aform = "%12.4f %12.4f %12.4f %8.3f %8.3f";
   }
   updir = (int) arriba; /* removed 'if (!extrap)' 8 July 94 */
   if (magx EQ 1) ixmag = 0;
   else ixmag = 1;
   if (magy EQ 1) iymag = 0;
   else iymag = 1;
