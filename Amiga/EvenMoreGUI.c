...
void Make_TS_Window(void)
{
 ...
 set(TS_Win->LonStr, MUIA_String_Contents, (IPTR)str);
 Set_TS_Reverse(0);
...
/* set active gadget */
 set(TS_Win->TimeSetWin, MUIA_Window_ActiveObject, (IPTR)TS_Win->TimeStr); 
...
double SunLon, SunLat, RefLon, SunTime;
 ...
 set(TS_Win->SunLatStr, MUIA_String_Contents, (IPTR)str); 
 sprintf(str, "%f", SunLon);
 set(TS_Win->SunLonStr, MUIA_String_Contents, (IPTR)str); 
...
double FloatDays, RefLon, SunTime;
 ...
 set(TS_Win->TimeStr, MUIA_String_Contents, (IPTR)str);
...
/* Set active gadget */
  set(PN_Win->NewProjWin, MUIA_Window_ActiveObject, (IPTR)PN_Win->Str[0]);
