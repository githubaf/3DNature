...
//#define WORD_VERBOSE // Displays sentances as added
//#define WORD_REALLY_VERBOSE // Displays all sentance subvariants as added
#define NUMBERIZE
...
/*				printf("WORD: \"%s\"\n", WordBuf); */
		...
		printf("DEBUG: %d = %d + %d + %d.\n\n", MainTable.TotalWords,
		 MainTable.Duplicates, MainTable.Rejects,
		 MainTable.UniqueWords);
...
/*		printf("DEBUG: OutName = '%s'.\n", OutName); */
		if (OutStream = fopen(OutName, "w"))
			...
			if(MainTable.TotalNodes < 65536)
				{
				strmfn(OutName, Drive, Path, "VocabTable", "cpp");
				if(OutStream = fopen(OutName, "w"))
					{
				...
				if(TableError)
					...
					strmfn(OutName, Drive, Path, "GrammarTable", "cpp");
					if(OutStream = fopen(OutName, "w"))
						{
						fprintf(OutStream, "// GrammarTable.cpp\n//\n// C++ Source for binary Markov word-sentance table.\n");
						fprintf(OutStream, "// Machine generated code, do NOT hand-alter.\n");
						fprintf(OutStream, "// Created from file \"%s\".\n\n\n", Vector[1]);
						fprintf(OutStream, "#define EXT extern\n");
						fprintf(OutStream, "#include \"Script.h\"\n\n\n");
						...
						{
						printf("ERROR: Could not open GrammarTable.cpp file for writing.\n");
						} /* else */
#endif /* NUMBERIZE */
					} /* else */
	...
	printf("\"VocabTable.cpp\" and \"GrammarTable.cpp\" in the same directory as\n");
		...
		if(This->Child)
			...
			while(*Scan) /* If I'm wrong, we'll get enforcer hits here. But I'm not. */
				...
				if((*Scan)->WordToken == New->WordToken)
					...
					New = *Scan;
					res = StageTwo(FuncName, New, Index);
					return(res);
					} /* if */
...
#ifdef WORD_REALLY_VERBOSE
int Loop;
#endif /* WORD_REALLY_VERBOSE */
...
if(Sentance[Index + 1])
	...
	if(Sentance[Index + 1] & RECOG_OPTIONAL)
		{
		#ifdef WORD_REALLY_VERBOSE
		printf("...{%s} ", DeRefWord(Sentance[Index + 1]));
		//printf("{} ");
		for(Loop = Index + 2; Loop < 10; Loop++)
			{
			if(Sentance[Loop])
				{
				if(Sentance[Loop] & RECOG_OPTIONAL)
	...
	if((Index == 0) && (Sentance[Index] & RECOG_OPTIONAL))
		{
		#ifdef WORD_REALLY_VERBOSE
		printf(" {%s} ", DeRefWord(Sentance[Index]));
		//printf("{} ");
		for(Loop = Index + 1; Loop < 10; Loop++)
			{
			if(Sentance[Loop])
				{
				if(Sentance[Loop] & RECOG_OPTIONAL)
		...
		#endif /* WORD_REALLY_VERBOSE */
	   if(WordTableAdd(FuncName, &CommandRoot, Index + 1)) /* Plus 1 skips us */
	      {
	      return(1);
	      } /* if */
		} // if 
