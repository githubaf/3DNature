...
short readDEM(char *filename, struct elmapheaderV101 *map)
{
 ...
 if (! error)
  ...
  sprintf(Str,"AF: sizeof(struct elmapheaderV101)=%ld\n",sizeof(struct elmapheaderV101));
  KPrintF((STRPTR)Str);


  sprintf(Str,"AF: rows=%ld\n",map->rows);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: columns=%ld\n",map->columns);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: lolat=%f\n",map->lolat);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF"
		  ": lolong=%f\n",map->lolong);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: steplat=%f\n",map->steplat);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: steplong=%f\n",map->steplong);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: elscale=%f\n",map->elscale);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: MaxEl=%d\n",map->MaxEl);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: MinEl=%d\n",map->MinEl);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: Samples=%ld\n",map->Samples);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: SumElDif=%f\n",map->SumElDif);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: SumElDifSq=%f\n",map->SumElDifSq);
  KPrintF((STRPTR)Str);

  //short	*map;
  //long	*lmap;

  sprintf(Str,"AF: size=%ld\n",map->size);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: scrnptrsize=%ld\n",map->scrnptrsize);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: fractalsize=%ld\n",map->fractalsize);
  KPrintF((STRPTR)Str);
  ...
  sprintf(Str,"AF: facept[0]=%ld\n",map->facept[0]);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: facept[1]=%ld\n",map->facept[1]);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: facept[2]=%ld\n",map->facept[2]);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: facect=%ld\n",map->facect);        //   <--- Falsch! Amiga: AF: facect=1079533704  AROS: AF: facect=-173681528
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: Lr=%ld\n",map->Lr);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: Lc=%ld\n",map->Lc);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: MapAsSFC=%d\n",map->MapAsSFC);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: ForceBath=%d\n",map->ForceBath);
  KPrintF((STRPTR)Str);

  sprintf(Str,"AF: LonRange=%f\n",map->LonRange);
  KPrintF((STRPTR)Str);
  sprintf(Str,"AF: LatRange=%f\n",map->LatRange);
  KPrintF((STRPTR)Str);

  KPrintF((STRPTR)"Calling AvailMem() am Ende von RedDEM()\n");
  AvailMem(MEMF_CLEAR);
  KPrintF((STRPTR)"Hinter AvailMem() am Ende von RedDEM()\n");
