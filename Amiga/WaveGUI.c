 ...
 if (WinNum == 0)
  ...
  Obj = VGroup,
		...
		Child, WV_Win->FloatStr[1] = StringObject, StringFrame,
			MUIA_FixWidthTxt, "01234567",
			MUIA_String_Accept, "-.0123456789", End,
		Child, WV_Win->FloatArrow[1][0] = ImageButtonWCS(MUII_ArrowLeft),
		Child, WV_Win->FloatArrow[1][1] = ImageButtonWCS(MUII_ArrowRight),
		End, /* HGroup */
	      End, /* ColGroup */
     ...
     case 3:				/* ID_WV_ADDWAVE(WinNum): */
      ...
      if (GUIWave_Add(WV_Win, WV_Win->WD, WV_Win->CurWave))
       ...
       if (WinNum == 1 && CL_Win)
        ...
        set(CL_Win->Text, MUIA_Text_Contents, (IPTR)str);
	} /* if */
   ...
   case GP_STRING3:
    ...
    if (WV_Win->TL)
     ...
     if (WV_Win->WKS.IsKey >= 0)
      ...
      set(WV_Win->TL->FrameTxt, MUIA_Text_Contents, (IPTR)str);
      TL_Redraw(WV_Win->TL);
      }/* if key frame */
 ...
 if (WKS && GKS)
  ...
  if (WKS->PrevKey >= 0)
   ...
   if (TL)
    ...
    set(TL->BT_PrevKey, MUIA_Text_Contents, (IPTR)str);
  ...
  if (WKS->NextKey >= 0)
   ...
   if (TL)
    ...
    set(TL->BT_NextKey, MUIA_Text_Contents, (IPTR)str);
  ...
  if (TL)
   ...
   set(TL->CycleStr, MUIA_String_Contents, (IPTR)str);
