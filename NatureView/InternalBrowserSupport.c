...
// taken from http://www.iol.ie/~locka/mozilla/control.htm
...
HRESULT STDMETHODCALLTYPE Frame_SetStatusText(IOleInPlaceFrame FAR* This, LPCOLESTR pszStatusText)
{
	StatusTextUpdateCallback(pszStatusText);
	return(S_OK);
}
...
void UnEmbedBrowserObject(HWND hwnd)
{
	...
	// Bad boy!
	//_ASSERT(0);
	// We'll let you get away with it quietly
}
...
{	
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		switch (action)
		{
			case NVW_INTERNALBROWSERSUPPORT_GOBACK:
			{
			...
			case NVW_INTERNALBROWSERSUPPORT_GOFORWARD:
			{
			...
			case NVW_INTERNALBROWSERSUPPORT_GOHOME:
			{
			...
			case NVW_INTERNALBROWSERSUPPORT_SEARCH:
			{
...
long FetchTitle(HWND hwnd, char *TitleBuffer, int TitleBufferSize)
{
	...
	// Retrieve the browser object's pointer we stored in our window's GWL_USERDATA when
	// we initially attached the browser object to this window.
	if(browserObject = *((IOleObject **)GetWindowLong(hwnd, GWL_USERDATA)))
	{
		...
		if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			// Ok, now the pointer to our IWebBrowser2 object is in 'webBrowser2', and so its VTable is
			// webBrowser2->lpVtbl.
			// Call the get_LocationName() function to fetch the title.
			webBrowser2->lpVtbl->get_LocationName(webBrowser2, &Title);
			...
			//if (!(buffer = (wchar_t *)GlobalAlloc(GMEM_FIXED, sizeof(wchar_t) * size))) goto badalloc;
			//MultiByteToWideChar(CP_ACP, 0, webPageName, -1, buffer, size);
			//Title.bstrVal = SysAllocString(buffer);
			//GlobalFree(buffer);
			WideCharToMultiByte(CP_ACP, 0, Title, -1, TitleBuffer, TitleBufferSize, NULL, NULL);
			}
	#else
			//Title.bstrVal = SysAllocString(webPageName);
	#endif
...
long FetchURL(HWND hwnd, char *URLBuffer, int URLBufferSize)
{
	...
	if(browserObject = *((IOleObject **)GetWindowLong(hwnd, GWL_USERDATA)))
	{
		...
		if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			// webBrowser2->lpVtbl.
			// Call the get_LocationName() function to fetch the title.
			webBrowser2->lpVtbl->get_LocationURL(webBrowser2, &URL);
			...
			//Title.bstrVal = SysAllocString(buffer);
			//GlobalFree(buffer);
			WideCharToMultiByte(CP_ACP, 0, URL, -1, URLBuffer, URLBufferSize, NULL, NULL);
			}
	#else
			//Title.bstrVal = SysAllocString(webPageName);
	#endif
...
/******************************* SetToolbar() ****************************
 * Enable or disable the toolbar. Doesn't seem to work right now.
 */
...
long SetToolbar(HWND hwnd, int ToolbarState)
{
	...
	if(browserObject = *((IOleObject **)GetWindowLong(hwnd, GWL_USERDATA)))
	{
		...
		if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			// Call the put_ToolBar() function to fetch the title.
			webBrowser2->lpVtbl->put_ToolBar(webBrowser2, ToolbarState);
...
// Retrieve the browser object's pointer we stored in our window's GWL_USERDATA when
// we initially attached the browser object to this window.
if(GWLResult = GetWindowLong(hwnd, GWL_USERDATA)) // safety checking
	{
	browserObject = *((IOleObject **)GWLResult);
 ...
 *
 * hwnd =			Handle to the window hosting the browser object.
 * Position =			left and top are coordinates in parent Window client coordinates
 *                      right and bottom are width/height coordinates _relative_ to those.
 *
 * NOTE: EmbedBrowserObject() must have been successfully called once with the
 * specified window, prior to calling this function. You need call
 * EmbedBrowserObject() once only, and then you can make multiple calls to
...
void ResizeBrowserRECT(HWND hwnd, RECT Position)
{
IWebBrowser2	*webBrowser2;
IOleObject		*browserObject;
LONG GWLResult;
...
if(GWLResult = GetWindowLong(hwnd, GWL_USERDATA)) // safety checking
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		...
		// Call put_*() to set the new position.
		webBrowser2->lpVtbl->put_Left(webBrowser2, Position.left);
		webBrowser2->lpVtbl->put_Top(webBrowser2, Position.top);
		webBrowser2->lpVtbl->put_Width(webBrowser2, Position.right);
		webBrowser2->lpVtbl->put_Height(webBrowser2, Position.bottom);
...
long EmbedBrowserObject(HWND hwnd, RECT *ClientPosition)
{
	IOleObject			*browserObject;
	IWebBrowser2		*webBrowser2;
	RECT				rect;
...
//	if (!OleCreate(&CLSID_MozillaBrowser, &IID_IOleObject, OLERENDER_DRAW, 0, (IOleClientSite *)_iOleClientSiteEx, &MyIStorage, (void**)&browserObject))
	{
		...
		*((IOleObject **)ptr) = browserObject;
		...
		browserObject->lpVtbl->SetHostNames(browserObject, L"NatureView", 0);

		if(ClientPosition != NULL)
			{
			UseWhich = ClientPosition;
			} // if
		else
			{
		...
		if (!OleSetContainedObject((struct IUnknown *)browserObject, TRUE) &&
			...
			!browserObject->lpVtbl->DoVerb(browserObject, OLEIVERB_SHOW, NULL, (IOleClientSite *)_iOleClientSiteEx, -1, hwnd, UseWhich) &&
			...
			!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			webBrowser2->lpVtbl->put_Left(webBrowser2, UseWhich->left);
			webBrowser2->lpVtbl->put_Top(webBrowser2, UseWhich->top);
			webBrowser2->lpVtbl->put_Width(webBrowser2, UseWhich->right);
			webBrowser2->lpVtbl->put_Height(webBrowser2, UseWhich->bottom);
