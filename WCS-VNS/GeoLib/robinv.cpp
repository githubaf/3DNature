...
yy = 2.0 * y / (PI * R);		// <<<F2 OPT>>>
phid = yy * 90.0;
p2 = fabs(phid * (1.0 / 5.0));	// <<<F2 OPT>>>
ip1 = (long) (p2 - EPSLN);
if (ip1 == 0)
	ip1 = 1;
...
for (i = 75; ; )
	...
	if ((p2 >= 0.0) || (ip1 == 1))
		...
		do
			{
			p2 = fabs(phid * (1.0 / 5.0));
			ip1 = (long) (p2 - EPSLN);
			p2 -= (double) ip1;

			if (y >= 0)
				y1 = R * (pr[ip1 +2] + p2 * (pr[ip1 + 3] - pr[ip1 +1]) * (1.0 / 2.0) + p2 * p2 *
				(pr[ip1 + 3] - 2.0 * pr[ip1 + 2] + pr[ip1 + 1]) * (1.0 / 2.0)) * PI * (1.0 / 2.0); 
			else
				y1 = -R * (pr[ip1 +2] + p2 * (pr[ip1 + 3] - pr[ip1 +1]) * (1.0 / 2.0) + p2 * p2 *
				(pr[ip1 + 3] - 2.0 * pr[ip1 + 2] + pr[ip1 + 1]) * (1.0 / 2.0)) * PI * (1.0 / 2.0); 
			phid += -180.0 * (y1 - y) / (PI * R);	// <<<F2 OPT>>>
			//i++;
			//if (i > 75)
			--i;	// F2_MOD: 071702 - faster loops
			if (i == 0)
				{
...
// calculate  LONG. using final LAT. with transposed forward Stirling's interpolation formula.
//*lon = lon_center + x / R / (xlr[ip1 + 2] + p2 * (xlr[ip1 + 3] - xlr[ip1 + 1]) / 2.0 +
//							 p2 * p2 * (xlr[ip1 + 3] - 2.0 * xlr[ip1 + 2] + xlr[ip1 + 1]) / 2.0);
// <<<F2 OPT>>>
*lon = lon_center + x / (R * (xlr[ip1 + 2] + p2 * (xlr[ip1 + 3] - xlr[ip1 + 1]) * (1.0 / 2.0) +
							 p2 * p2 * (xlr[ip1 + 3] - 2.0 * xlr[ip1 + 2] + xlr[ip1 + 1]) * (1.0 / 2.0)));
*lon = adjust_lon(*lon);
