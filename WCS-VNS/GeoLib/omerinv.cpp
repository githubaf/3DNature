...
es = 1.0 - SQUARE(temp);								// Snyders e^2 (eccentricity squared)
e = sqrt(es);											// Snyders e (eccentrity)

sincos(lat_origin, &sin_p20, &cos_p20);
con = 1.0 - es * sin_p20 * sin_p20;						// denominator in equation 9-12
com = sqrt(1.0 - es);									// numerator in equation 9/12
bl = sqrt(1.0 + es * pow(cos_p20, 4.0) / (1.0 - es));	// Snyders B (9-11)
al = r_major * bl * scale_factor * com / con;			// Snyders A (9-12)
...
if (mode != 0)
	...
	gama = atan(sin(bl * dlon) / g);											// Snyders gamma-sub-0
	azimuth = asinz(d * sin(gama));												// Snyders alpha-sub-c
...
InverseOK[WCS_PROJECTIONCODE_HOM] = 1;
InverseTransform = &Projectoid::omerinv;
...
y -= false_northing;
flag = 0;
vs = x * cosaz - y * sinaz;				// Snyders v
us = y * cosaz + x * sinaz;				// Snyders u
us = us + u;							// why a modified u???
q = exp(-bl * vs / al);					// Snyders Q'
s = .5 * (q - 1.0 / q);					// Snyders S'
t = .5 * (q + 1.0 / q);					// Snyders T'
vl = sin(bl * us / al);					// almost Snyders V' - different due to modified u
ul = (vl * cosgam + s * singam) / t;	// almost Snyders U' - different due to modified u
