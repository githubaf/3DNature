...
radius(r);
ForwardOK[WCS_PROJECTIONCODE_GOOD] = 1;
ForwardTransform = &Projectoid::goodfor;
...
if (region==1 || region==3 || region==4 || region==5 || region==8 || region==9)
	{
	delta_lon = adjust_lon(lon - lon_center12[region]);
	*x = feast12[region] + R * delta_lon * cos(lat);
	*y = R * lat;
	}
else
	{
	...
	if (HALF_PI - fabs(lat) < EPSLN)	// <<<F2 OPT>>>
		delta_lon = 0;
	*x = feast12[region] + 0.900316316158 * R * delta_lon * cos(theta);
