...
InverseOK[WCS_PROJECTIONCODE_UTM] = 1;
InverseTransform = &Projectoid::utminv;
...
double ie0;							// <<<F2 OPT>>> inverse e0
long max_iter = 6;					// maximun number of iterations
unsigned long i;					// counter variable
...
//for (i=0;;i++)
for (i = max_iter; ; --i)	// F2_MOD: 071702 - faster loops
	{
	delta_phi=((con + e1 * sin(2.0 * phi) - e2 * sin(4.0 * phi) + e3 * sin(6.0 * phi)) * ie0) - phi;	// <<<F2 OPT>>>
...
if (fabs(phi) < HALF_PI)
	...
	d    = x / (n * scale_factor);
	ds   = SQUARE(d);
	*lat = phi - (n * tan_phi * ds / r) * (0.5 - ds * (1.0 / 24.0) * (5.0 + 3.0 * t + 
		  10.0 * c - 4.0 * cs - 9.0 * esp - ds * (1.0 / 30.0) * (61.0 + 90.0 * t +
		  298.0 * c + 45.0 * ts - 252.0 * esp - 3.0 * cs)));
	*lon = adjust_lon(lon_center + (d * (1.0 - ds * (1.0 / 6.0) * (1.0 + 2.0 * t +
		  c - ds * (1.0 / 20.0) * (5.0 - 2.0 * c + 28.0 * t - 3.0 * cs + 8.0 * esp +
		  24.0 * ts))) / cos_phi));
	}
else
	{
