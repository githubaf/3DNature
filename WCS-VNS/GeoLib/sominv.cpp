...
T. Mittan               Mar,  1993
F. Weed II				Jan   2001		som_series() return value changed to void
...
c1 = sumc1 / 15.0;
c3 = sumc3 / 45.0;
InverseOK[WCS_PROJECTIONCODE_SOM] = 1;
InverseTransform = &Projectoid::sominv;
...
for (inumb = 50; ; --inumb)	// F2_MOD: 071702 - faster loops
	{
	sav = tlon;
	sd = sin(tlon);
	sdsq = sd * sd;
	...
	if (fabs(dif) < conv)
		break;
	if (inumb == 0)	// F2_MOD: 071702 - faster loops
		{
...
st = sin(tlon);
defac = exp(sqrt(1.0 + s * s / (xj * xj)) * (y / a - c1 * st - c3 * sin(3.0 * tlon)));	// <<<F2 OPT>>>
actan = atan(defac);
tlat = 2.0 * (actan - (PI * (1.0 / 4.0)));	// <<<F2 OPT>>>
...
if (cos(tlon) < 0.0)
	scl= -1.0;
xlamt = xlamt - ((PI * (1.0 / 2.0)) * (1.0 - scl) * sl);
dlon = xlamt - p21 * tlon;
...
void Projectoid::som_series(double *fb, double *fa2, double *fa4, double *fc1, double *fc3, double *dlam)
{
const double dlam_r = *dlam * 0.0174532925;	// <<<F2 OPT>>>
double sd, sdsq, h, sq, fc;

//*dlam = *dlam * 0.0174532925;              // Convert dlam to radians
sd = sin(dlam_r);
sdsq = sd * sd;
s = p21 * sa * cos(dlam_r) * sqrt((1.0 + t * sdsq) / ((1.0 + w * sdsq) * (1.0 + q * sdsq)));
h = sqrt((1.0 + q * sdsq) / (1.0 + w * sdsq)) * (((1.0 + w * sdsq) / ((1.0 + q * sdsq) * (1.0 + q * sdsq))) - p21 * ca);
sq = sqrt(xj * xj + s * s);
*fb = (h * xj - s * s) / sq;
*fa2 = *fb * cos(2.0 * dlam_r);	// <<<F2 OPT>>>
*fa4 = *fb * cos(4.0 * dlam_r);	// <<<F2 OPT>>>
fc = s * (h + xj) / sq;
*fc1 = fc * cos(dlam_r);		// <<<F2 OPT>>>
*fc3 = fc * cos(3.0 * dlam_r);	// <<<F2 OPT>>>
*dlam = dlam_r;					// <<<F2 OPT>>>
