...
if (id == 1)
	...
	false_east = table[7];
	false_north = table[8];
	lamccforint(r_maj, r_min, lat1, lat2, center_lon, center_lat, false_east, false_north);
	}
else if (id == 3)
	{
	center_lon = paksz(pakcz(table[2]), &iflg) * D2R;
	...
	false_north = table[8];
	mode = 1;
	omerforint(r_maj, r_min, scale_fact, azimuth, lon_orig, lat_orig, false_east, false_north, lon1, lat1, lon2, lat2, mode);
	}
ForwardOK[WCS_PROJECTIONCODE_SPCS] = 1;
ForwardTransform = &Projectoid::stplnfor;
