...
ForwardOK[WCS_PROJECTIONCODE_OBEQA] = 1;
ForwardTransform = &Projectoid::obleqfor;
...
Az = atan2(cos_lat * sin_delta_lon , cos_lat_o * sin_lat - sin_lat_o * cos_lat * cos_delta_lon) + theta;
sincos(Az, &sin_Az, &cos_Az);
temp = 2.0 * sin(z * (1.0 / 2.0));	// <<<F2 OPT>>>
x_prime = temp * sin_Az;
y_prime = temp * cos_Az;
M = asin(x_prime * (1.0 / 2.0));	// <<<F2 OPT>>>
temp = y_prime * (1.0 / 2.0) * cos(M) / cos(2.0 * M / m);	// <<<F2 OPT>>>
N = asin(temp);
