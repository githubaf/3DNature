...
ForwardOK[WCS_PROJECTIONCODE_VGRINT] = 1;
ForwardTransform = &Projectoid::vandgfor;
...
theta = asinz(2.0 * fabs(lat * INV_PI));	// <<<F2 OPT>>>
...
al = .5 * fabs((PI / dlon) - (dlon * INV_PI));	// <<<F2 OPT>>>
asq = al * al;
...
m = g * (2.0 / sinth - 1.0);
msq = m * m;
con = PI * R * (al * (g - msq) + sqrt(asq * (g - msq) * (g - msq) - (msq + asq) * (gsq - msq))) / (msq + asq);
if (dlon < 0)
	con = -con;
*x = false_easting + con;
con = fabs(con / (PI * R));
