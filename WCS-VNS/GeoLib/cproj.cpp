...
if (eccent > 1.0e-7)
	{
	con = eccent * sinphi;
	value = ( 1.0 - eccent * eccent) * (sinphi / (1.0 - con * con) - (0.5 / eccent) * log((1.0 - con) / (1.0 + con)));
	}
else
	value = 2.0 * sinphi;
...
//for (i = 1; i <= 25; i++)
for (i = 26; i > 0; --i)	// F2_MOD: 071702 - faster loops
	{
...
//for (i = 0; i <= 15; i++)
for (i = 16; i > 0; --i)	// F2_MOD: 071702 - faster loops
	{
	sinpi = sin(phi);
	con = eccent * sinpi;
...
// m1, e0, e1, e2, e3 = Constants
// *flag = Error flag number
{
const double ie0 = 1.0 / e0;	// <<<F2 OPT>>>
double phi;
double dphi;
unsigned long i;
...
for (i = 16; i > 0; --i)	// F2_MOD: 071702 - faster loops
	{
	dphi = (ml + e1 * sin(2.0 * phi) - e2 * sin(4.0 * phi) + e3 * sin(6.0 * phi)) * ie0 - phi;	// <<<F2 OPT>>>
	phi += dphi;
...
// F2: Changed to long return value 071602
//double phi4z(double eccent, double e0, double e1, double e2, double e3, double a, double b, double *c, double *phi)
...
*phi = a;
//for (i = 1; i <= 15; i++)
for (i = 15; i > 0; --i)	// F2_MOD: 071702 - faster loops
	{
	sinphi = sin(*phi);
	tanphi = tan(*phi);
...
if (pak < 0.0)
	sgna = '-';
con = fabs(pak);
degs = (long)((con * (1.0 / 10000.0)) + .001);
con =  con  - degs * 10000;
mins = (long)((con * (1.0 / 100.0)) + .001);
secs = con  - mins * 100;
...
// eccent;        /* Eccentricity of the spheroid         */
// phi;           /* Latitude phi                         */
// sinphi;        /* Sine of the latitude                 */
{
double con;
double com;
...
// F2_MOD: 071702 - assigned a return type
//sign(double x)
long sign(double x)
{
...
{
//long count = 0;
long count = MAX_VAL;	// F2_MOD: 071702 - faster loops

for (;;)
	{
	if (fabs(x) <= PI)
		break;
	else if (((long) fabs(x * INV_PI)) < 2)	// <<<F2 OPT>>>
		x = x - (sign(x) * TWO_PI);
	else if (((long) fabs(x * INV_TWO_PI)) < MAXLONG)	// <<<F2 OPT>>>
		{
		x = x - (((long)(x * INV_TWO_PI)) * TWO_PI);	// <<<F2 OPT>>>
		}
...
return(1.0 - 0.25 * x * (1.0 + x * (1.0 / 16.0) * (3.0 + 1.25 * x)));
...
return((long)(((lon + 180.0) * (1.0 / 6.0)) + 1.0));
