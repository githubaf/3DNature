...
InverseOK[WCS_PROJECTIONCODE_LAMAZ] = 1;
InverseTransform = &Projectoid::lamazinv;
...
if (fabs(Rh) > EPSLN)
	...
	*lat = asinz(sin_lat_o * cos_z + cos_lat_o * sin_z * y / Rh);
	...
	if (fabs(temp) > EPSLN)
		...
		if (temp != 0.0)
			*lon = adjust_lon(lon_center + atan2(x * sin_z * cos_lat_o, temp * Rh));
		}
