...
lon_center12[0] = -1.74532925199;		// -100.0 degrees
lon_center12[1] = -1.74532925199;		// -100.0 degrees
lon_center12[2] =  0.523598775598;		//   30.0 degrees
lon_center12[3] =  0.523598775598;		//   30.0 degrees
lon_center12[4] = -2.79252680319;		// -160.0 degrees
lon_center12[5] = -1.0471975512;		//  -60.0 degrees
lon_center12[6] = -2.79252680319;		// -160.0 degrees
lon_center12[7] = -1.0471975512;		//  -60.0 degrees
lon_center12[8] =  0.349065850399;		//   20.0 degrees
lon_center12[9] =  2.44346095279;		//  140.0 degrees
lon_center12[10] = 0.349065850399;		//   20.0 degrees
lon_center12[11] = 2.44346095279;		//  140.0 degrees

// Initialize false eastings for each of the 12 regions
feast12[0] = R * -1.74532925199;
feast12[1] = R * -1.74532925199;
feast12[2] = R * 0.523598775598;
feast12[3] = R * 0.523598775598;
feast12[4] = R * -2.79252680319;
feast12[5] = R * -1.0471975512;
feast12[6] = R * -2.79252680319;
feast12[7] = R * -1.0471975512;
feast12[8] = R * 0.349065850399;
feast12[9] = R * 2.44346095279;
feast12[10] = R * 0.349065850399;
feast12[11] = R * 2.44346095279;

// Report parameters to the user
ptitle("GOODE'S HOMOLOSINE EQUAL-AREA");
radius(r);
InverseOK[WCS_PROJECTIONCODE_GOOD] = 1;
InverseTransform = &Projectoid::goodinv;
...
if (region==1 || region==3 || region==4 || region==5 || region==8 || region==9)
	...
	*lat = y / R;
	...
	if (fabs(temp) > EPSLN)
		{
		temp = lon_center12[region] + x / (R * cos(*lat));
		*lon = adjust_lon(temp);
		}
	else *lon = lon_center12[region];
	}
else
	{
	...
	if (fabs(arg) > 1.0)
		return(IN_BREAK);
	theta = asin(arg);
	*lon = lon_center12[region] + ( x / (0.900316316158 * R * cos(theta)));
	if (*lon < -(PI + EPSLN))
		return(IN_BREAK);
...
// because of precision problems, long values of 180 deg and -180 deg may be mixed.
if (((x < 0) && (PI - *lon < EPSLN)) || ((x > 0) && (PI + *lon < EPSLN))) *lon = -(*lon);
