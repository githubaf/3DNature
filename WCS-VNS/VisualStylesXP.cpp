...
// Too many of these warnings to deal with in this file
//lint -save -e611
...
HMODULE CVisualStylesXP::LoadThemeRcDll(void)
{
	if (m_hRcDll==NULL)
	{
		...
		if(GetCurrentThemeName(szThemeFileName,MAX_PATH,szColorBuff,MAX_PATH,szSizeBuff,MAX_PATH) == S_OK)
		{
			m_hRcDll=::LoadLibraryW(szThemeFileName);
		}
	}
	return(m_hRcDll);
}
...
HBITMAP CVisualStylesXP::LoadBitmapFromTheme(LPCWSTR pszBitmapName)
{
	HBITMAP hbmRet = NULL;
	WCHAR szBitmapFilename[MAX_PATH];
	if(LoadThemeRcDll())
	{
		wcscpy(szBitmapFilename, pszBitmapName);
		while(wchar_t *Replace = wcschr(szBitmapFilename, '\\')) Replace[0] = '_';
		while(wchar_t *Replace = wcschr(szBitmapFilename, '.')) Replace[0] = '_';
		if (m_hRcDll != NULL)
			hbmRet = ::LoadBitmapW(m_hRcDll, szBitmapFilename); // specifically wide-character
	}
	return hbmRet;
}
...
HRESULT CVisualStylesXP::GetThemeBitmap(HTHEME hTheme, HDC hdc, 
											 int iPartId, int iStateId, int iPropId, ULONG dwFlags, HBITMAP *phBitmap)
{
	PFNGETTHEMEBITMAP pfnGetThemeBitmap = 
		(PFNGETTHEMEBITMAP)GetProc("GetThemeBitmap", (void*)GetThemeBitmapFail);
	return (*pfnGetThemeBitmap)(hTheme, hdc, iPartId, iStateId, iPropId, dwFlags, phBitmap);
}
