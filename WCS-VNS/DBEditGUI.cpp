...
#include "WidgetSupport.h" // for layout assistance functions
...
// for testing, make unconditional later if we like it
static bool ShowSQComponents;
...
static NotifyTag AllAppExEvents[] = {MAKE_ID(WCS_NOTIFYCLASS_FREEZE, 0xff, 0xff, 0xff),
									...
									MAKE_ID(WCS_EFFECTSSUBCLASS_SEARCHQUERY, 0xff, 0xff, 0xff),
...
ActiveColorEditItem = -1;
IndividualEditInProgress = false;
...
ShowSQComponents = GlobalApp->MainProj->Prefs.PrivateQueryConfigOptTrue("dbedit_show_sq_components");
...
switch (ButtonID)
	...
	case IDC_ADDLAYER:
		...
		if (ActivePage == 4)
			...
			Qualifier = LocalWinSys()->CheckQualifier(WCS_GUI_KEYCODE_CONTROL) ? 2: 0;
			if (Qualifier)
				// link selection set together as parts for hole cutting
				DoLinkAttr();
			else
				// normal attributes
				DoAddAttr();
	...
	case IDI_DEMPAINT:
		...
		if (CurrentJoe = GetJoeFromOBN(GetActiveObject()))
			...
			if (CurrentJoe->TestFlags(WCS_JOEFLAG_ISDEM))
				...
				ReDrawList(); // force list to redraw -- sometimes it doesn't after opening a new window
				} // if
			} // if
		break;
		} // 
...
char Redraw = 0, Rebuild = 0, ClearList = 0, Reconfig = 0, RebuildEffects = 0, Done = 0, LayerChange = 0, TestLoop;
...
if (TestChange = GlobalApp->AppEx->MatchNotifyClass(Interested, Changes, 0))
	...
	if (MatchClass > WCS_EFFECTSSUBCLASS_LAKE && MatchClass < WCS_MAXIMPLEMENTED_EFFECTS)
		...
		Reconfig = 1;
		Redraw = 1;
		if (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_ENABLED)
			{
			Rebuild = 1; // may affect our list contents if we're filtering by this criteria
			} // if
		Done = 1;
		} // else if
	} // if thaw
...
if (TestChange = GlobalApp->AppEx->MatchNotifyClass(Interested, Changes, 1))
	{
	if (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_QUERY)
		{
		if (QueryFilter == Activity->ChangeNotify->NotifyData)
			{
			Rebuild = 1;
			} // if
		} // if
...
WidgetSetCheck(IDC_SHOWPROPPANEL, ShowPropertiesPanel);
...
void DBEditGUI::UpdateWidgets(void)
{
Joe *CurrentJoe;
char Second = 0;
...
if (LayerToSelect)
	...
	for (j = 0; j < NoOfObjects; j ++)
		...
		if ((CurJoe = GetJoeFromOBN(j)) && CurJoe->MatchLayer(LayerToSelect))
			...
			// don't set busy until after first object is selected, so DBE responds to change and scrolls grid properly
			ImBusy = 1;
...
if (MyLayer)
	...
	if (! MyLayer->TestFlags(WCS_LAYER_LINKATTRIBUTE))
		...
		for (j = 0; j < NoOfObjects; j ++)
			{
			// do "Item" item if IndividualEditInProgress, otherwise do selection set
			if (((GetSelected(j) && !IndividualEditInProgress) || (j == Item && IndividualEditInProgress)) && (CurJoe = GetJoeFromOBN(j)))
				{
...
void DBEditGUI::DoLinkAttr(void)
{
#ifdef WCS_SUPPORT_GENERIC_ATTRIBS
long j, PartNum;
Joe *CurJoe;
LayerStub *Stubby;
LayerEntry *TargetEntry = NULL; 
NotifyTag Changes[2];
char TargetName[200];

Changes[1] = NULL;
TargetName[0] = NULL;
// see if any of the selected vectors already have a link attribute - use it if they do
for (j = 0; j < NoOfObjects; j ++)
	{
	if (GetSelected(j) && (CurJoe = GetJoeFromOBN(j)))
		{
		if (TargetEntry = CurJoe->GetMultiPartLayer())
			break;
		} // if
	} // for
// add vectors to link attribute layer
for (j = PartNum = 0; j < NoOfObjects; j ++)
	{
	if (GetSelected(j) && (CurJoe = GetJoeFromOBN(j)))
		{
		if (! TargetEntry)
			{
			// no link attribute yet. so create one
			...
			if (Stubby = CurJoe->AddTextAttribute(name, TargetName))
				Stubby->MyLayer()->SetFlags(WCS_LAYER_LINKATTRIBUTE);
			TargetEntry = Stubby->MyLayer();
			} // if
		else if (! CurJoe->GetMultiPartLayer())
			CurJoe->AddObjectToLayer(TargetEntry);
		++PartNum;
		Changes[0] = MAKE_ID(CurJoe->GetNotifyClass(), CurJoe->GetNotifySubclass(), 0xff, WCS_NOTIFYCOMP_ATTRIBUTE_VALUECHANGED);
		GlobalApp->AppEx->GenerateNotify(Changes, CurJoe->GetRAHostRoot());
		} // if
...
if ((TotNumSel = ListView_GetSelectedCount(GetWidgetFromID(IDC_LIST1))) < 2)
	...
	if (TheDude)
		...
		ImBusy = 0;
		ConfigUIForActiveObject(CurDude); // doesn't change selection, just sets up UI for active selection
		} // if
	} // if
...
if (ListView_GetItem(GetWidgetFromID(IDC_LIST1), &LVI))
	...
	SetSelected(OBN, 1);
	// move it into view, like DBEdit used to
	ListView_EnsureVisible(GetWidgetFromID(IDC_LIST1), OBN, FALSE);
	} // if
ConfigUIForActiveObject(OBN);

} // DBEditGUI::SetActiveObject()

/*===========================================================================*/

void DBEditGUI::ConfigUIForActiveObject(long OBN)
{
...
} // DBEditGUI::ConfigUIForActiveObject()
...
if (GetInputString("Enter new Layer Name", "", LayerName))
	...
	if (LayerName[0])
		{
		// Freeze, other wise we lose grid widget selection state when we make the new layer
		Changes[0] = MAKE_ID(WCS_NOTIFYCLASS_FREEZE, WCS_NOTIFYSUBCLASS_FREEZE, 0xff, 0xff);
		Changes[1] = NULL;
		GlobalApp->AppEx->GenerateNotify(Changes, NULL);
...
void DBEditGUI::DoRemoveLink(void)
{
LayerEntry *LE = NULL;
NotifyTag Changes[2];
long j;
LayerEntry *MultiPartLayer = NULL;
...
for (j = 0; j < NoOfObjects; ++j)
	...
	if (GetSelected(j))
		...
		if (! MultiPartLayer)
			{
			MultiPartLayer = GetJoeFromOBN(j)->GetMultiPartLayer();
			} // if
		GetJoeFromOBN(j)->RemoveObjectFromLayer(TRUE, MultiPartLayer);
		Changes[0] = MAKE_ID(MultiPartLayer->GetNotifyClass(), MultiPartLayer->GetNotifySubclass(), 0xff, WCS_NOTIFYCOMP_ATTRIBUTE_COUNTCHANGED);
		Changes[1] = NULL;
		GlobalApp->AppEx->GenerateNotify(Changes, MultiPartLayer->GetRAHostRoot());
		} // if
	} // for
	
if (MultiPartLayer)
	{
	Host->DBLayers.RemoveLayer(MultiPartLayer);
	} // if
	
BuildLayerList(Host->ActiveObj, ListView_GetItemCount(GetWidgetFromID(IDC_LIST1)));

} // DBEditGUI::DoRemoveLink()
		...
		if (JD = (JoeDEM *)Iterate->MatchAttribute(WCS_JOE_ATTRIB_INTERNAL, WCS_JOE_ATTRIB_INTERNAL_DEM))
			...
			if (!JD->QueryCachedELEVDataValid())
				...
				if (!JD->UpdateCachedELEVData(Iterate, ProjHost))
					{
					if(!Iterate->TestFlags(WCS_JOEFLAG_NEWRETRYCACHE)) // is it newly-added this session? If so, keep retrying caching
						{ // otherwise, give up and disable the object if it can't be found
					...
					if (!BWCH)
						{
						BWCH = new BusyWin("Caching DEM Data", 10, 'BWCD', 0);
						} // if
					if (BWCH)
						{
...
//		if (1)
			...
			else if (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_ENABLED)
				{
				if (!Iterate->TestFlags(WCS_JOEFLAG_ACTIVATED)) FilterApproved = false;
				} // else if
...
int LayerListMode = WidgetGetCheck(IDC_SHOWALL);
...
if (ActivePage == 4)
	...
	if (LayerListMode != 1)
		...
		WidgetSetText(IDC_LAYERTEXT, "All Attribute fields in Project");
	} // if
else
	{
	if (LayerListMode != 1)
		...
		WidgetSetText(IDC_LAYERTEXT, "All Layers in Project");
	} // else
		...
		while (MyLayer)
			...
			if (MyName = MyLayer->GetName())
				...
				if (IsAttribute && ActivePage == 4)
					...
					if (! MyLayer->TestFlags(WCS_LAYER_LINKATTRIBUTE))
						{
						MyTurn = Host->ActiveObj ? Host->ActiveObj->MatchEntryToStub(MyLayer): NULL;
						if (MyLayer->TestFlags(WCS_LAYER_TEXTATTRIBUTE))
							{
							if (MyTurn)
								sprintf(LNameTest, "%s=%s", &MyName[1], MyTurn->GetTextAttribVal());
...
const char *DBEditGUI::ParseEffectNameClass(JoeAttribute *Me, GeneralEffect *Effect, bool DecorateWithPrefix)
{
long Place = 0;
...
if (Me)
	...
	switch (Me->MinorAttrib())
		...
		*/
			{
			const char *CombinedString = ParseEffectNameClass(Me, NULL, true);
...
if (GlobalApp->MainProj->Prefs.DBEFilterMode != WCS_ENUM_DBEDITGUI_FILTER_MODE_NONE)
	...
	switch (GlobalApp->MainProj->Prefs.DBEFilterMode)
		...
		case WCS_ENUM_DBEDITGUI_FILTER_MODE_ENABLED:
			{
			sprintf(FilterDescString, " [Filter: Enabled]");
			break;
...
if(ActiveColorEditItem == -1) return; // abort if we're not already in a color edit operation

DP.DoThreeShort = (void (*)(class Joe *,short,short,short))SetRGBPen;

ImBusy = 1;

// do the Active Edit item
if(IndividualEditInProgress)
	{
...
if ((ActivePage == 1) || (ActivePage == 0) || (!ShowPropertiesPanel)) // extent, properties
	...
	// determine Subpanel button height
	RECT SubPanelButtonSize;
	GetClientRect(GetWidgetFromID(IDC_SEL1), &SubPanelButtonSize); // How big is the Layers sub-panel's Select button
	
	// Calculate sub-panel size
	RECT GridWidgetPos;
	GetClientRectInParentCoords(GetWidgetFromID(IDC_LIST1), &GridWidgetPos);
	int NewSubPanelHeight = GridWidgetPos.bottom - ConvertCoord.y;

	// move the sub-panels
	int SubPanelX, SubPanelY;
	SubPanelX = ConvertCoord.x + NewWidgetWidth;
	SubPanelY = ConvertCoord.y;
	SetWindowPos(SubPanels[0][0], NULL, SubPanelX, SubPanelY, OriginalWidgetRect.right, NewSubPanelHeight, SWP_NOZORDER); // properties
	SetWindowPos(SubPanels[0][1], NULL, SubPanelX, SubPanelY, OriginalWidgetRect.right, NewSubPanelHeight, SWP_NOZORDER); // extent vec
	SetWindowPos(SubPanels[0][2], NULL, SubPanelX, SubPanelY, OriginalWidgetRect.right, NewSubPanelHeight, SWP_NOZORDER); // extent dem
	SetWindowPos(SubPanels[0][3], NULL, SubPanelX, SubPanelY, OriginalWidgetRect.right, NewSubPanelHeight, SWP_NOZORDER); // effect
	SetWindowPos(SubPanels[0][4], NULL, SubPanelX, SubPanelY, OriginalWidgetRect.right, NewSubPanelHeight, SWP_NOZORDER); // layers
	
	// bottom row of buttons in Layer/Attrib/Effect sub-panels
	MoveWindowY(GetWidgetFromID(IDC_ADDLAYER), NewSubPanelHeight - SubPanelButtonSize.bottom);
	MoveWindowY(GetWidgetFromID(IDC_REMOVELAYER), NewSubPanelHeight - SubPanelButtonSize.bottom);
	MoveWindowY(GetWidgetFromID(IDC_SHOWALL), NewSubPanelHeight - SubPanelButtonSize.bottom);
	MoveWindowY(GetWidgetFromID(IDC_MODIFYEFFECT), NewSubPanelHeight - SubPanelButtonSize.bottom);
	MoveWindowY(GetWidgetFromID(IDC_REMOVEEFFECT), NewSubPanelHeight - SubPanelButtonSize.bottom);

	// second row of buttons in Layer/Attrib sub-panels
	MoveWindowY(GetWidgetFromID(IDC_SEL1), NewSubPanelHeight - (SubPanelButtonSize.bottom + SubPanelButtonSize.bottom + 4));
	MoveWindowY(GetWidgetFromID(IDC_ON1), NewSubPanelHeight - (SubPanelButtonSize.bottom + SubPanelButtonSize.bottom + 4));
	MoveWindowY(GetWidgetFromID(IDC_OFF1), NewSubPanelHeight - (SubPanelButtonSize.bottom + SubPanelButtonSize.bottom + 4));
	MoveWindowY(GetWidgetFromID(IDC_SETATTR), NewSubPanelHeight - (SubPanelButtonSize.bottom + SubPanelButtonSize.bottom + 4));
	
	GetClientRectInParentCoords(GetWidgetFromID(IDC_LAYERLIST), &OriginalWidgetRect);
	SetWindowPos(GetWidgetFromID(IDC_LAYERLIST), NULL, 0, 0, OriginalWidgetRect.right - OriginalWidgetRect.left, (NewSubPanelHeight - (SubPanelButtonSize.bottom + SubPanelButtonSize.bottom + 8)) - OriginalWidgetRect.top, SWP_NOMOVE | SWP_NOZORDER); // layers/attribs
	SetWindowPos(GetWidgetFromID(IDC_EFFECTLIST), NULL, 0, 0, OriginalWidgetRect.right - OriginalWidgetRect.left, (NewSubPanelHeight - (SubPanelButtonSize.bottom + 4)) - OriginalWidgetRect.top, SWP_NOMOVE | SWP_NOZORDER); // effects
	} // else
...
// Create a Rebar to contain the toolbar
   hwndRB = CreateWindowEx(0, REBARCLASSNAME, (LPSTR) NULL, 
        WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | RBS_VARHEIGHT | RBS_AUTOSIZE | CCS_NORESIZE,
...
if (FilterPopupMenu = CreatePopupMenu())
	...
	AppendMenu(FilterPopupMenu, MF_STRING | MF_ENABLED | (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_ENABLED ? MF_CHECKED : 0), ID_DBEDIT_FILTER_MENU_ENABLED, "Enabled");
	if (LayerSub = CreatePopupMenu())
		...
		for (unsigned int LayerLoop = 0; LayerLoop < LayerColumns.size(); LayerLoop++)
			...
			UINT BreakFlag = NULL;
			bool Matched = false;
			if (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_LAYER && !stricmp(GetLayerColumn(LayerLoop)->GetName(), GlobalApp->MainProj->Prefs.DBELayerFilterName))
				{
				Matched = true;
				} // if
			BreakFlag = (LayerLoop % 25 == 0);
			AppendMenu(LayerSub, MF_STRING | MF_ENABLED | (Matched ? MF_CHECKED : 0) | (BreakFlag ? MF_MENUBREAK : NULL), ID_DBEDIT_FILTER_MENU_LAYERBASE + LayerLoop,
	...
	if (SQSub = CreatePopupMenu())
		...
		for (MyEffect = (SearchQuery *)HostLib->GetListPtr(WCS_EFFECTSSUBCLASS_SEARCHQUERY); MyEffect; MyEffect = (SearchQuery *)MyEffect->Next)
			...
			UINT BreakFlag = NULL;
			bool Matched = false;
			if (GlobalApp->MainProj->Prefs.DBEFilterMode == WCS_ENUM_DBEDITGUI_FILTER_MODE_QUERY && !stricmp(MyEffect->GetName(), GlobalApp->MainProj->Prefs.DBESearchQueryFilterName))
				{
				Matched = true;
				} // if
			BreakFlag = (Count % 25 == 0);
			AppendMenu(SQSub, MF_STRING | MF_ENABLED | (Matched ? MF_CHECKED : NULL) | (BreakFlag ? MF_MENUBREAK : NULL), ID_DBEDIT_FILTER_MENU_QUERYBASE + Count++, MyEffect->GetName());
			} // for
		} // if
	#endif // WCS_BUILD_VNS
	...
	if (ConformPopResult = TrackPopupMenu(FilterPopupMenu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_RIGHTBUTTON, CursorPos.x, CursorPos.y, 0, NativeWin, NULL))
		...
		switch (ConformPopResult)
			...
			case ID_DBEDIT_FILTER_MENU_ENABLED:
				{
				GlobalApp->MainProj->Prefs.DBEFilterMode = WCS_ENUM_DBEDITGUI_FILTER_MODE_ENABLED;
				break;
				} // ID_DBEDIT_FILTER_MENU_ENABLED
			default:
...
ListView_GetSubItemRect(MyControl, Item, WhichColumn, LVIR_LABEL, &CellRect);
X = CellRect.left - 1;
Y = CellRect.top - 2;
...
// this is used to determine which component types should be checked against Search Queries
...
const char *GridWidgetInstance::FetchTextForColumn(int Item, int WhichColumn, bool NoDecorations) // pass NoDecorations=true to get text for editing
{
...
if (JoeItem)
	...
	switch (WhichColumn)
		...
		case WCS_ENUM_DBEDITGUI_GRID_COLUMN_NAME:
			...
			*/
			ResultText = JoeItem->GetBestName();
			if(JoeItem->TemplateItem && !NoDecorations)
				{
				sprintf(ScratchFormatBuffer, "%s [Templated]", ResultText);
...
*/
		...
		case WCS_ENUM_DBEDITGUI_GRID_COLUMN_AREA:
			...
			sprintf(AssembleBuffer, "%.7f", Area);
				...
				if (JoeItem)
					...
					if (MyTurn = JoeItem->GetFirstAttribute())
						...
						while (MyTurn)
							...
							if (MyTurn->MajorAttrib() == WCS_JOE_ATTRIB_INTERNAL && MyTurn->MinorAttrib() != WCS_JOE_ATTRIB_INTERNAL_DEM)
								{
								if(MyTurn->MinorAttrib() == WCS_JOE_ATTRIB_INTERNAL_COORDSYS)
									{
									CS = MyTurn; // handle this last
									} // if
								else
									{
									const char *EffectNameClass;
									if (EffectNameClass = DBEHost->ParseEffectNameClass(MyTurn, NULL, false))
										{
						...
						if(ShowSQComponents) // this could be slow
							{
							for(long EffectType = 0; JoeLinkableList[EffectType]; EffectType ++) // iterate all effect types that can be joe-linked
								{
								for (GeneralEffect *CurrentEffect =HostLib->GetListPtr(JoeLinkableList[EffectType]); CurrentEffect; CurrentEffect = CurrentEffect->Next) // iterate all components of that type
									{
									if(SearchQuery *MyEffect = CurrentEffect->Search) // does the current Component have a SQ?
										{
										if (MyEffect->ApproveJoe(JoeItem)) // does it link to this Joe?
											{
											const char *EffectNameClass;
											if (EffectNameClass = DBEHost->ParseEffectNameClass(NULL, CurrentEffect, false))
												{
												if (OneAdded) StrAssemble += ", ";
												StrAssemble += EffectNameClass;
												OneAdded = true;
						...
						if(CS) // handle CS now
							{
							const char *EffectNameClass;
							if (EffectNameClass = DBEHost->ParseEffectNameClass(CS, NULL, false))
								{
								if (OneAdded) StrAssemble += ", ";
								StrAssemble += EffectNameClass;
								OneAdded = true;
...
switch (WhichColumn)
	...
	case WCS_ENUM_DBEDITGUI_GRID_COLUMN_NAME:
		{
		if (Joe *JoeItem = DBEHost->GetJoeFromOBN(Item))
			{
			// what we change here follows the same rules as Joe::GetBestName()
			if (JoeItem->Name())
				{ // label
				if(DBEHost->IndividualEditInProgress)
					{
					ChangeNames(JoeItem, ProjHost, NewText, NULL); // do the item we clicked
					} // else
				else
					{
			...
			else if (JoeItem->FileName())
				{ // name
				if(DBEHost->IndividualEditInProgress)
					{
					ChangeNames(JoeItem, ProjHost, NULL, NewText); // do the item we clicked
					} // else
				else
					{
			...
			else // CanonName, SecondaryName, other
				{ // put edited name into Label
				if(DBEHost->IndividualEditInProgress)
					{
					ChangeNames(JoeItem, ProjHost, NewText, NULL); // do the item we clicked
					} // else
...
*/
	...
	default:
		...
		if (DBEHost->IsColumnAnAttribute(WhichColumn))
			{
			// ChangeAttribValue handles the changed item and selection set in one whack, dealing with IndividualEditInProgress as well
...
switch (message)
	...
	case WM_WCSW_GW_LVNBLEREFLECT: // LVN_BEGINLABELEDIT reflected from parent window for subclassing
		{ // we don't use the built-in label editing, so cancel it
		if (WThis && WThis->GWStyles & WCSW_GW_STYLE_JOE)
			{
			Joe *JoeItem = NULL;
			NMLVDISPINFO *pdi = (NMLVDISPINFO *) lParam;
				...
				if (WThis && WThis->GWStyles & WCSW_GW_STYLE_JOE)
					...
					if (JoeItem)
						...
						switch (CDraw->iSubItem)
							...
							case WCS_ENUM_DBEDITGUI_GRID_COLUMN_COLOR:
								{ // drawn in postpaint so that we cover the highlight color
								if (!JoeItem->TestFlags(WCS_JOEFLAG_ISDEM))
									{
									RECT CellRect;
									ListView_GetSubItemRect(hwnd, CDraw->nmcd.dwItemSpec, CDraw->iSubItem, LVIR_BOUNDS, &CellRect);
									//CDraw->clrTextBk = WINGDI_RGB(JoeItem->Red(), JoeItem->Green(), JoeItem->Blue());
									HBRUSH JoeColor = CreateSolidBrush(WINGDI_RGB(JoeItem->Red(), JoeItem->Green(), JoeItem->Blue()));
									FillRect(CDraw->nmcd.hdc, &CellRect, JoeColor);
									DeleteObject(JoeColor);
									} // if
								break;
								} // color
							default:
			...
			case CDDS_SUBITEM | CDDS_ITEMPREPAINT:
				...
				if (WThis->GWStyles & WCSW_GW_STYLE_GREENBAR) // actually light-blue bar
					...
					if (Selected)
						...
						CDraw->clrTextBk = GetSysColor(COLOR_HIGHLIGHT); // 0x00c56a31 on stock WinXP
						CDraw->clrText = 0x00ffffff; // make reversed white text always
				...
				if (WThis && WThis->GWStyles & WCSW_GW_STYLE_JOE)
					...
					if (JoeItem)
						...
						switch (CDraw->iSubItem)
							...
							case WCS_ENUM_DBEDITGUI_GRID_COLUMN_COLOR:
								{
								// moved to postpaint so it covers the highlight color
								SetWindowLong(GetParent(hwnd), DWL_MSGRESULT, (LONG)CDRF_NOTIFYPOSTPAINT);
								break;
								} // color
							default:
		...
		switch (Notify)
			...
			case EN_KILLFOCUS:
				...
				if (WThis->ActiveEditColumn != WCS_ENUM_DBEDITGUI_GRID_COLUMN_NONE && WThis->EditControl && Edit_GetModify(WThis->EditControl))
					...
					if(!WThis->DBEHost->GetSelected(WThis->ActiveEditItem))
						{
						WThis->DBEHost->IndividualEditInProgress = true;
						} // if
					else
						{
						WThis->DBEHost->IndividualEditInProgress = false;
						} // else
		...
		if (HitItem != -1)
			...
			if (JoeItem)
				...
				if (HitTest.iSubItem == WCS_ENUM_DBEDITGUI_GRID_COLUMN_NAME/*  || HitTest.iSubItem == WCS_ENUM_DBEDITGUI_GRID_COLUMN_LABEL */)
					{
					std::string TempString = WThis->FetchTextForColumn(HitItem, HitTest.iSubItem, true); // pass NoDecorations to eliminate [Templated] decoration
					WThis->CreateEdit(HitItem, HitTest.iSubItem, false, TempString.c_str());
					} // iSubItem
				else if(HitTest.iSubItem == WCS_ENUM_DBEDITGUI_GRID_COLUMN_MAX) // component column
					{ // try to edit the most logical component
					JoeAttribute *MyTurn, *Best = NULL, *CS = NULL;
					if (MyTurn = JoeItem->GetFirstAttribute())
						{
						bool OneAdded = false;
						while (MyTurn)
							...
							if (MyTurn->MajorAttrib() == WCS_JOE_ATTRIB_INTERNAL && MyTurn->MinorAttrib() != WCS_JOE_ATTRIB_INTERNAL_DEM)
								{
								if(MyTurn->MinorAttrib() == WCS_JOE_ATTRIB_INTERNAL_COORDSYS)
									{ // we prefer non-CS components, but record it in case we don't find anything better
									CS = MyTurn;
									} // if
								else if(!Best)
									{ // first non-CS is considered best
									Best = MyTurn;
									} // else
								} // if
							MyTurn = MyTurn->GetNextAttribute();
						...
						if(!Best && CS)
							{ // we'll take a CS if nothing else is available
							Best = CS;
							} // if
						} // if
					if(Best)
						{
						if(GeneralEffect *BestComponent = Best->GetGeneralEffect())
							{
							BestComponent->Edit();
							} // if
						} // if
	...
	case WM_LBUTTONDOWN:
		...
		if (HitItem != -1)
			...
			if (JoeItem)
				{
				// determine if HitItem is outside the selection set, and if so, set IndividualEditInProgress true
				if(!WThis->DBEHost->GetSelected(HitItem))
					{
					WThis->DBEHost->IndividualEditInProgress = true;
					} // if
				else
				...
				switch (HitTest.iSubItem)
					...
					case WCS_ENUM_DBEDITGUI_GRID_COLUMN_ENABLED:
						...
						if(WThis->DBEHost->IndividualEditInProgress)
							{
							if (NewState) JoeItem->SetFlags(WCS_JOEFLAG_ACTIVATED);
							else JoeItem->ClearFlags(WCS_JOEFLAG_ACTIVATED);
							// we may need to fire our own notify if there weren't any items in the selection set
							Changes[1] = NULL;
							if (JoeItem->TestFlags(WCS_JOEFLAG_ISDEM))
								{
								Changes[0] = MAKE_ID(WCS_RAHOST_OBJTYPE_DEM, 0xff, 0xff, WCS_NOTIFYCOMP_OBJECT_ENABLEDCHANGED);
								...
								{
								if (JoeItem->TestFlags(WCS_JOEFLAG_ISCONTROL))
									{
									Changes[0] = MAKE_ID(WCS_RAHOST_OBJTYPE_CONTROLPT, 0xff, 0xff, WCS_NOTIFYCOMP_OBJECT_ENABLEDCHANGED);
									} // if
								else
					...
					case WCS_ENUM_DBEDITGUI_GRID_COLUMN_COLOR:
						{
						// edit color
						// color edit callbacks handle IndividualEditInProgress situation
						WThis->DBEHost->ActiveColorEditItem = -1; // this is never cleared except here, and needs to be cleared before ValueChanged() below
					...
					case WCS_ENUM_DBEDITGUI_GRID_COLUMN_CLASS:
						{
						// combobox callbacks handle IndividualEditInProgress situation
					...
					case WCS_ENUM_DBEDITGUI_GRID_COLUMN_WEIGHT:
						...
						// edit callbacks handle IndividualEditInProgress situation
...
*/
					...
					//case WCS_ENUM_DBEDITGUI_GRID_COLUMN_MAX: // component
						{ // read-only text
						// read-only fields ignore IndividualEditInProgress situation
						std::string TempString = WThis->FetchTextForColumn(HitItem, HitTest.iSubItem);
						if (!TempString.empty())
							{
					...
					default:
						...
						if (WThis->DBEHost->IsColumnALayer(HitTest.iSubItem))
							...
							if (Matched = JoeItem->MatchLayer(ColumnLayer))
								...
								if(WThis->DBEHost->IndividualEditInProgress)
									{
									if (!(JoeItem->MatchLayer(ColumnLayer)))
										{
							...
							// edit callbacks handle IndividualEditInProgress situation
							SetFocus(NULL); // cause KILLFOCUS on any existing edit fields so their changes "take"
							std::string TempString = WThis->FetchTextForColumn(HitItem, HitTest.iSubItem);
							WThis->CreateEdit(HitItem, HitTest.iSubItem, false, TempString.c_str());
							} // else if
						break;
