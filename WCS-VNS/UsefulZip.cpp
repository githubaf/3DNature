...
ZipFileMember* Zipper::AddToZipList(const char *RelativePathAndFile)
{
ZipFileMember *AddMe, *Current;
//char dmsg[1024];

//strcpy(dmsg, RelativePathAndFile);
//strcat(dmsg, "\n");
//DEBUGOUT(dmsg);

if (AddMe = new ZipFileMember)
	{
	if (Current = GetFirst())
		{
		while (Current->Next)
		...
		Current = First = AddMe;
	strncpy(Current->RelativePathAndName, RelativePathAndFile, ZIPPER_MAXPATHNAMELEN_MINUSONE);
	Current->RelativePathAndName[ZIPPER_MAXPATHNAMELEN_MINUSONE] = 0;
	} // if AddMe
...
bool Zipper::NotInZipList(const char *relativePathAndFile)
{
ZipFileMember *Current;
bool rVal = true;
...
if (Current = GetFirst())
	...
	while (Current)
		{
		if (Current->RelativePathAndName && relativePathAndFile && (strcmp(Current->RelativePathAndName, relativePathAndFile) == 0))
			{
			rVal = false;
			break;
			} // if
...
void Zipper::SetBaseDirectory(const char *BaseDirectory)
{

strncpy(BasePathAndName, BaseDirectory, ZIPPER_MAXPATHNAMELEN_MINUSONE);
BasePathAndName[ZIPPER_MAXPATHNAMELEN_MINUSONE - 1] = 0;

} // Zipper::SetBaseDirectory

/*===========================================================================*/

void Zipper::SetRemoveStateOnAll(bool NewRemoveState)
{
ZipFileMember *Current;
...
while (Current)
	{
	Current->RemoveAfterAdd = NewRemoveState;
	Current = Current->GetNext();
	} // while

} // Zipper::SetRemoveStateOnAll

/*===========================================================================*/

int Zipper::ZipToFile(char *NewFilePathAndName)
{
int rc = 0, size_buf = 0;
void *buf = NULL;
ZipFileMember *Current;
zipFile zf;
char msg[512];
char FullPathAndName[ZIPPER_MAXPATHNAMELEN * 2];

size_buf = WRITEBUFFERSIZE;
buf = (void *)AppMem_Alloc(size_buf, 0);
if (buf == NULL)
	{
	GlobalApp->StatusLog->PostError(WCS_LOG_SEVERITY_MSG, "Archiving failed - Can't allocate memory for buffer!");
	} // if buf == NULL

if (buf && (zf = zipOpen(NewFilePathAndName, 0)))
	{
	int err = 0;
	zip_fileinfo zi;
	...
	while (Current)
		...
		time(&rightnow);
		newtime = localtime(&rightnow);
		zi.tmz_date.tm_hour = newtime->tm_hour;
		zi.tmz_date.tm_mday = newtime->tm_mday;
		zi.tmz_date.tm_min = newtime->tm_min;
		zi.tmz_date.tm_mon = newtime->tm_mon;
		zi.tmz_date.tm_sec = newtime->tm_sec;
		zi.tmz_date.tm_year = newtime->tm_year - 80;	// convert from 1900 to 1980 start time

		strcpy(FullPathAndName, BasePathAndName);
		strcat(FullPathAndName, Current->RelativePathAndName);
		err = zipOpenNewFileInZip(zf, Current->RelativePathAndName, &zi,
			NULL, 0, NULL, 0, NULL, Z_DEFLATED, 9);
		if (err != ZIP_OK)
			rc = 2;
		else
			{
			fin = fopen(FullPathAndName, "rb");
			if (fin == NULL)
				{
				err = ZIP_ERRNO;
				sprintf(&msg[0], "Can't open %s", FullPathAndName);
		...
		if (err == ZIP_OK)
			...
			do
				...
				if (size_read < size_buf)
					if (feof(fin) == 0)
						{
						err = ZIP_ERRNO;
						sprintf(&msg[0], "Error reading %s", FullPathAndName);
						GlobalApp->StatusLog->PostError(WCS_LOG_SEVERITY_MSG, msg);
						rc = 4;
						} // if feof == 0
				...
				if (size_read > 0)
					...
					if (err < 0)
						{
						sprintf(&msg[0], "Error writing %s to zipfile", FullPathAndName);
						GlobalApp->StatusLog->PostError(WCS_LOG_SEVERITY_MSG, msg);
						rc = 5;
						} // if err < 0
					} // if size_read > 0
			...
			if (err < 0)
				...
				if (err != ZIP_OK)
					{
					sprintf(&msg[0], "Can't close %s", FullPathAndName);
					GlobalApp->StatusLog->PostError(WCS_LOG_SEVERITY_MSG, msg);
					rc = 6;
					} // if err != ZIP_OK
				} // else
    ...
    if (err != ZIP_OK)
		{
		sprintf(&msg[0], "Unable to close %s", NewFilePathAndName);
		GlobalApp->StatusLog->PostError(WCS_LOG_SEVERITY_MSG, msg);
		rc = 6;
		} // if
	else
		{
		// archive created OK, so remove any flagged files
		Current = GetFirst();
		while (Current)
			{
			if (Current->RemoveAfterAdd)
				{
				strcpy(FullPathAndName, BasePathAndName);
				strcat(FullPathAndName, "/");
				strcat(FullPathAndName, Current->RelativePathAndName);
				remove(FullPathAndName);
				} // if
			Current = Current->Next;
			} // while
		} // else

	AppMem_Free(buf, size_buf);
	} // if zf
else
	rc = -1;
...
#define Z_MAXFILENAMEINZIP (256)	//lint -e750
#endif
...
#define FLAG_LOCALHEADER_OFFSET (0x06)	//lint -e750
#define CRC_LOCALHEADER_OFFSET  (0x0e)	//lint -e750
...
/*** Lint points out that this is never referenced
...
local int ziplocal_putValue (FILE *file, uLong x, int nbByte)
{
    unsigned char buf[4];
    int n;
	memset(buf, 0, 4);	// Lint sees buf as possibly unitialized, so just initialize it
    for (n = 0; n < nbByte; n++) {
...
int ZEXPORT zipOpenNewFileInZip(zipFile file, const char* filename, const zip_fileinfo* zipfi, const void* extrafield_local,
...
int ZEXPORT zipWriteInFileInZip(zipFile file, const voidp buf, unsigned len)
{
    ...
    zi->ci.crc32 = crc32(zi->ci.crc32,(const unsigned char *)buf,len);
